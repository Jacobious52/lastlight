...
Motion atlases use eight columns, six rows per cycle, and at most 3072 pixels in either dimension. Runtime rendering needs one artwork sample per animated silhouette; motion estimation runs offline, never in the browser.
...
The traveller rig uses the intact neutral boot and shin rather than moving fragments of a different coat pose. Two-bone inverse kinematics preserves bone lengths; rigid rotations/translations preserve the painted pieces. Upper-body placement bending and idle breathing do not deform the boots.
